...
public class Chunkdata implements Serializable {
    ...
    public enum Event {
        ...
        public int categorizeEvent() {
			return switch (this) {
				case PLAYER_ENTERS, PLAYER_LEAVES -> 1;
				default -> 0;
			};
        }
    ...
    public class ChunkView implements Iterable<EventView> {
        ...
        @Override
        public @NotNull Iterator<EventView> iterator() {
            return new Iterator<>() {
				int i;
    ...
    public class MapView implements Iterable<ChunkView> {
        ...
        @Override
        public @NotNull Iterator<ChunkView> iterator() {
            return new Iterator<>() {
				int i = 0;
    ...
    public void addData(int gametick, int eventNumber, int x, int z, int d, int eventcode, int traceid, int reason) {
        try {
			/*
			make log file hard to read
            if (allStacktraces.get(traceid) == null) {
                System.err.printf("Warning: Referenced non-existant Stacktrace %d (All Stacktraces: %d)%n", traceid, allStacktraces.size());
            }
    ...
    private static class EventCollection {
        TreeMap<Integer, FullEvent[]> eventsForGametick = new TreeMap<>();
        TreeMap<FullEvent, FullEvent[]> eventsForChunk;

        EventCollection() {
            eventsForChunk = new TreeMap<>(spacialSorted);
        }
    ...
    private FullEvent[][] getAllChunksForGametick(int gametick, int dimension, int minx, int maxx, int minz, int maxz) {
        ...
        for (int zi = 0; zi < zsize; ++zi) {
            ...
            while (startindex < thisGametick.length) {
				...
				while (endindex < thisGametick.length && spacialSorted.compare(thisGametick[endindex], event) == 0) {
					endindex++;
				}
    ...
    private static final Comparator<FullEvent> spacialSorted = (e1, e2) -> {
		if (e1.d != e2.d) {
    ...
    private static final Comparator<FullEvent> temporalSorted = (e1, e2) -> {
		if (e1.t != e2.t) {
    ...
    private void unpackNBTStackTrace(NbtCompound nbt) {
		NbtList nbtlist = nbt.getList("stackList", 10);

        for (int i = 0; i < nbtlist.size(); ++i) {
            NbtCompound nbtcompound = nbtlist.getCompound(i);
            int id = nbtcompound.getInt("id");
            String stack = nbtcompound.getString("stack");
