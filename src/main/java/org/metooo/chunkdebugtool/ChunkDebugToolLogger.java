...
public class ChunkDebugToolLogger {
	//this class is a single instance class
	public static ChunkDebugToolLogger logger = new ChunkDebugToolLogger();
	//enables or disables the sending process
	public boolean enabled = false;
	//the chunk event reason
	public static String reason = null;
	public static String oldReason = null;
	//the stacktrace cache
	...
	//several ways to change the reason for async events
	public static void resetToOldReason() {	reason = oldReason;}
	public static void setReason(String r) {
		oldReason = reason;
		reason = r;
	}
	public static void resetReason() {reason = null;}
	/*
	...
	private ArrayList<ChunkLog> getInitialChunksForNewClient(MinecraftServer server) {
		...
		for (ServerWorld w : server.worlds) {
			...
			for (WorldChunk c : provider.getLoadedChunks()) {
				...
				if (provider.isChunkLoaded(c.chunkX, c.chunkZ)) {
	...
	private static class StackTraces {
		...
		private String asString(StackTraceElement[] trace, boolean deobfuscated) {
			...
			for (i = 0; i < trace.length && i < size; i++) {
				...
				if ("ChunkDebugToolLogger.java".equals(e.getFileName())) {
					continue;
				}
	...
	private record InternedString(int id, String obfuscated, String deobfuscated) {
		@Override
			public int hashCode() {
				return id;
			}
		}
	//A log class for a chunk
	...
	private class ChunkLoggerSerializer {
		...
		private void sendInitialChunks(ServerPlayerEntity sender) {
			MinecraftServer server = sender.getServer();
			ArrayList<ChunkLog> logs;
			if (server != null) {
				logs = getInitialChunksForNewClient(server);
				...
				for (int i = 0; i < logs.size(); i += LOGS_BATCH_SIZE) {
					...
					if (chunkData != null) {
						ChunkDebugToolHandler.sendNBTChunkData(sender,PACKET_EVENTS, chunkData);
					}
				}
			}
			...
			for (int i = 0; i < logs.size(); i += LOGS_BATCH_SIZE) {
				...
				if (chunkData != null) {
					for (ServerPlayerEntity player : this.sentTracesForPlayer.keySet()) {
						ChunkDebugToolHandler.sendNBTChunkData(player, PACKET_EVENTS,chunkData);
					}
				}
			}
		}
		...
		private void sendMissingStackTracesForPlayer(ServerPlayerEntity player, ArrayList<ChunkLog> events) {
			...
			for (int i = 0; i < missingList.size(); i += STACKTRACES_BATCH_SIZE) {
				...
				if (stackData != null) {
					ChunkDebugToolHandler.sendNBTChunkData(player, PACKET_STACKTRACE,stackData);
				}
			}
		}
		...
		private void kickAllPlayers() {
			for (ServerPlayerEntity player : this.sentTracesForPlayer.keySet()) {
				ChunkDebugToolHandler.sendNBTChunkData(player,PACKET_ACCESS_DENIED, new NbtCompound());
			}
